...
export default function ThoughtSpace({ thoughts, filter, user, handleUpdateThoughts }: ThoughtSpaceProps) {
    ...
    function Reaction(reaction: Reaction, depth: number, thoughtId: string,) {
        return (
            <>
                ...
                {selectedId === reaction._id && <input className="border-2 border-slate-600  text-black p-1 w-96" placeholder="Reaction..." type="text" style={{ marginLeft: `${(depth + 1) * 50}px` }}
    ...
    return (
        <section className="bg-slate-200 dark:bg-slate-900 dark:text-slate-200 mt-12 pt-5 text-lg">
            ...
            {selectedId == '0' && <input className=" shadow-slate-500 shadow-sm p-1 w-96 text-black" placeholder="Reaction..." type="text" onBlur={createThought}></input>}
                ...
                {thoughts.map(thought => (
                    <div key={thought._id}>
                        ...
                        {selectedId === thought._id &&
                            <input className=" shadow-slate-500 shadow-sm p-1 w-96 text-black" placeholder="Reaction..." type="text" style={{ marginLeft: `${1 * 50}px` }} onBlur={(e) => postReactionToThought(e, thought._id)}></input>
                        }
