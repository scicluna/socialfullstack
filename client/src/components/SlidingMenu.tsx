    ...
    return (
        <div>
            <button onClick={() => { hamburger ? setHamburger(false) : setHamburger(true) }}>
                <FontAwesomeIcon icon={icon} size='3x' className="text-slate-600 hover:text-slate-900" />
            </button>
            <div className={`fixed bg-slate-300 h-screen ${direction == 'right' ? 'top-0 right-0' : 'top-0 left-0'} transition-all ease-in duration-200 ${hamburger ? "w-64" : "w-0"} `}>
